...
class BattleMaster(object):
    ...
    def filt_challenge_of_day(challenge_list, time, zone_num: int = 8):
        ...
        return list(
            filter(lambda challen: day == BattleMaster.get_yyyymmdd(challen['time'], zone_num)[2], challenge_list))
    ...
    def stat_score(self, cid, time):
        ...
        ret = [
            (mem['uid'], mem['alt'], mem['name'],
             sum(map(lambda ch: round(self.get_score_rate(ch['round'], ch['boss'], server) * ch['dmg']), challens)))
            for mem, challens in stat
        ]
        return ret
